...
export default function ChangePasswordForm() {
  ...
  return <div className="passwordPanel"><button className="passwordToggle" type="button" onClick={()=>setOpen(!open)}>{open?"TUTUP":"GANTI PASSWORD"}</button>{open&&<form onSubmit={submit}><label>Password lama<input name="currentPassword" type="password" required autoComplete="current-password"/></label><label>Password baru<input name="newPassword" type="password" required minLength={8} autoComplete="new-password"/></label><label>Ulangi password baru<input name="confirmation" type="password" required minLength={8} autoComplete="new-password"/></label>{error&&<p className="passwordError">{error}</p>}{message&&<p className="passwordSuccess">{message}</p>}<button disabled={loading}>{loading?"MENYIMPAN...":"SIMPAN PASSWORD"}</button></form>}</div>;
}
