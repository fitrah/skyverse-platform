...
export async function POST(_:Request,{params}:Params){
  ...
  try{
    ...
    const players=await client.query<{user_id:string}>("SELECT user_id::text FROM memory_room_players WHERE room_id=$1 ORDER BY seat",[state.id]);
    if(players.rows.length<2)throw Object.assign(new Error("Minimal 2 pemain untuk mulai."),{status:409});
    ...
    await client.query("UPDATE memory_rooms SET status='playing',current_turn_user_id=$2,updated_at=now() WHERE id=$1",[state.id,players.rows[0].user_id]);
