...
type CatalogGame = { title: string; description: string; game_url: string; project_id: string | null };

export default async function Home() {
  const user = await getCurrentUser();
  const result = await query<CatalogGame>("SELECT title,description,game_url,project_id::text FROM games WHERE status='published' ORDER BY created_at DESC LIMIT 12");
